...
def build_crew(user_message):
    ...
    agent = Agent(
        ...
        goal="Answer customer questions with 100% accuracy using only live Firebase data.",
        backstory=(
            "You are the Northstar inventory assistant. Source of truth: "
            f"firebase_inventory_tool. Follow this guide: {dt}"
        ),
        tools=[tool],
        verbose=True,
    ...
    task = Task(
        description=(
            ...
            "1. Classify. 2. Call firebase_inventory_tool. "
            "3. Parse response. 4. Compose reply."
        ),
        expected_output="A complete customer-facing reply grounded in live Firebase data.",
        agent=agent,
    )
